...
(function (window) {
  ...
  function createXSocialHTML() {
    ...
    const existingScreen = document.getElementById('x-social-screen');
    const phoneScreen = document.querySelector('.phone-screen');
    if (phoneScreen && !phoneScreen.contains(existingScreen)) {
      phoneScreen.appendChild(existingScreen);
    }
    return;
  }
  ...
  container.className = 'screen'; // 使用宿主应用的 screen 类
container.style.cssText = 'background-color: #000; color: #fff; display: none; flex-direction: column; height: 100%;';

  // HTML 结构 (只修改返回按钮的 onclick 事件)
  container.innerHTML = `
    <!-- 顶部栏 -->
    <div class="x-top-bar"
      ...
      <!-- 返回按钮 -->
      <div class="x-back-btn" onclick="showScreen('home-container')" style="cursor: pointer;">
  ...
  if (phoneScreen) {
    ...
    console.log('✅ X Social App: HTML结构已创建并附加到 .phone-screen');
  } else {
    document.body.appendChild(container); // Fallback
    console.warn('⚠️ 未找到 .phone-screen, X Social App 已附加到 body');
  }
  ...
  const StringBuilders = {
    // 构建角色信息字符串
      ...
      let info = `\n角色名：${char.remarkName}`; // 使用 remarkName
      info += `\n本名：${char.realName}`; // 使用 realName
      info += `\n人设：${char.persona || '无特定人设'}`; // 使用 persona
      ...
      if (char.history && char.history.length > 0) {
        ...
        recentHistory.forEach(msg => {
          if (msg.role === 'assistant' && msg.content) {
            info += `\n- ${char.remarkName}: ${msg.content.substring(0, 100)}...`; // 使用 remarkName
          }
        });
      }
    // 构建用户身份识别信息
...
async buildCompleteCharacterInfo(boundCharacters, userXProfileInfo, scenario = 'reply') {
      ...
      const mainDB = getDB(); // 获取宿主应用的 window.db
      const xDB = getXDB(); 
      
      if (!mainDB || !mainDB.characters) { // 检查 db.characters
        console.warn("宿主应用的 db.characters 未找到");
        return '';
      }
async buildCompleteCharacterInfo(boundCharacters, userXProfileInfo, scenario = 'reply') {
      if (!boundCharacters || boundCharacters.length === 0) return '';
...
// 获取绑定角色的详细信息
      ...
      for (const char of boundCharsData) {
        ...
        if (!xProfile) {
          // 如果没有设置X资料，使用默认值
          xProfile = {
            xName: char.remarkName,
            xHandle: char.remarkName.toLowerCase().replace(/\s+/g, '_'),
            xAvatar: char.avatar, // 使用 avatar
